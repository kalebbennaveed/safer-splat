...
from cbf.si_cbf_utils import CBF_SINGLE
from dynamics.systems import  SingleIntegrator, single_integrator_dynamics
...
# method = 'ball-to-ellipsoid-single-integrator'
...
for scene_name in ['stonehenge']:
    for method in ['ball-to-ellipsoid-single-integrator']:
        ...
        for trial, (start, goal) in enumerate(zip(x0, xf)):
            ...
            # State is 3D. First 3 are position
            ...
            for i in tqdm(range(n_steps), desc=f"Simulating trajectory {trial}"):
                ...
                # Propagate dynamics
                x_ = x.clone()
                x = single_integrator_dynamics(x,u)*dt + x
                ...
                if scene_name == 'flight-low-res':
                    ...
                    h, grad_h, hess_h, info = gsplat_high_res.query_distance(x, radius=radius, distance_type='ball-to-ellipsoid-single-integrator')
                else:
                    h, grad_h, hess_h, info = gsplat.query_distance(x, radius=radius, distance_type='ball-to-ellipsoid-single-integrator')
